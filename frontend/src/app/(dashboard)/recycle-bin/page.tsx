...
export default function RecycleBinPage() {
  ...
  const handleRestore = (id: string) => {
    // Simulasi memanggil endpoint POST /restore/:id (Soft Delete Reversal)
    setRestoredId(id);
    setTimeout(() => {
      setItems(items.filter(item => item.id !== id));
      setRestoredId(null);
    }, 1500); // Simulasi delay API 1.5 detik
  };
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 dark:bg-zinc-800/50 text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-zinc-800">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Dihapus Oleh</th>
                ...
                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-500">
                    <div className="flex items-center gap-2">
                      <Clock size={14} /> {new Date(item.deletedAt).toLocaleString('id-ID')}
                    </div>
                  </td>
